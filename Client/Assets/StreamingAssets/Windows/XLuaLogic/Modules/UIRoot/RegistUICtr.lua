...
require("Windows.XLuaLogic.Entity.RegistTestEntity")
RegistUICtr = {};
...
function RegistUICtr.OnLoadItemView(itemViewPrefab)
    ...
    table[#table+1] = RegistTestEntity.New(1002,"武器考验",1,"试试这把无尽之刃吧,简直是神器");
    table[#table+1] = RegistTestEntity.New(1003,"技能传授",2,"看我神威,无坚不摧");
    table[#table+1] = RegistTestEntity.New(1004,"修为精进",0,"修为精进,一日千里,真是万中无一的绝世高手");
    table[#table+1] = RegistTestEntity.New(1005,"强化武器",1,"一把无尽不大够,再来一把电刀如何...");
    table[#table+1] = RegistTestEntity.New(1006,"魔法抗性",0,"现在你需要一件幽魂斗篷来保护自己");
    table[#table+1] = RegistTestEntity.New(1007,"物理抗性",2,"没有比荆棘之甲更能保护你了");
    table[#table+1] = RegistTestEntity.New(1008,"春哥馈赠",0,"信春哥得永生 ,春哥宝甲送给你了,可以复活哦");
    table[#table+1] = RegistTestEntity.New(1009,"移速暴增",2,"穿上轻灵之靴,健步如飞,日行千里");
    ...
    for i = 1, #table do
        ...
        oo.transform:Find("Status"):GetComponent("UnityEngine.UI.Text").text = this.ShowState( table[i].Status);
        ...
        tog.onValueChanged:AddListener(function (isOn)         
            ...
            if isOn then 
                ...
                RegistUIView.txt_Name.text = table[i].Title;
            end
...
function RegistUICtr.ShowState(Status)
	if Status == 0 then 
