...
public sealed class SpawnPool : MonoBehaviour
{
    ...
    private void Awake()
    {
        ...
        foreach (PrefabPool prefabPool in this._perPrefabPoolOptions)
        {
            ...
            {
                PPP.pppShow(true,string.Format("Initialization Warning: Pool '{0}' "+
    ...
    /// 
    /// Note: Instances with ParticleSystems can be preloaded too because 
    ...
    public void Add(Transform instance, string prefabName, bool despawn, bool parent)
    {
        foreach (PrefabPool prefabPool in this._prefabPools)
        {
            ...
            {
                PPP.pppShow(true,"Unexpected Error: PrefabPool.prefabGO is null");
                return;
            }
        ...
        PPP.pppShow(true,string.Format("SpawnPool {0}: PrefabPool {1} not found.",
                                     this.poolName,
                                     prefabName));
    ...
    /// the scene that is queued for reuse. Type = ParticleSystem
    /// </param>
    /// <param name="pos">The position to set the instance to</param>
    /// <param name="quat">The rotation to set the instance to</param>
    /// <returns>
    /// The instance's ParticleSystem. 
    /// 
    /// If the Limit option was used for the PrefabPool associated with the
    /// passed prefab, then this method will return null if the limit is
    /// reached. You DO NOT need to test for null return values unless you 
                                 ...
                                 Vector3 pos, Quaternion quat)
    {
		...
		//var animator = inst.GetComponent<ParticleSystem>();
    ...
    /// <returns>The instance's ParticleSystem</returns>
    public ParticleSystem Spawn(ParticleSystem prefab,
                                 Vector3 pos, Quaternion quat,
    ...
    public void Despawn(Transform xform, PrefabPool prefabPool, bool setInActive = true, bool callDeSpawned = true)
    {
        ...
        if (!despawned)
        {
            PPP.pppShow(true,string.Format("SpawnPool {0}: {1} not found in SpawnPool",
                           this.poolName,
                           xform.name));
           return;
        }
    ...
    public Transform GetPrefab(Transform prefab)
    {
        ...
        {
            if (prefabPool.prefabGO == null)
                PPP.pppShow(true,string.Format("SpawnPool {0}: PrefabPool.prefabGO is null",
                                             this.poolName));
    ...
    /// </summary>
    /// <param name="emitter">ParticleSystem to process</param>
    /// <returns></returns>
...
public class PrefabPool
{
    ...
    internal List<Transform> PreloadInstances()
    {
        ...
        if (this.prefab == null)
        {
            PPP.pppShow(true,string.Format("SpawnPool {0} ({1}): Prefab cannot be null.",
                                         this.spawnPool.poolName,
                                         this.prefab.name));
        ...
        while (this.totalCount < this.preloadAmount) // Total count will update
        {
            ...
            if (inst == null)
            {
                PPP.pppShow(true,string.Format("SpawnPool {0} ({1}): " +
